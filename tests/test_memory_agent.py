...
def test_memory_agent_models_backpressure_latency_and_masked_write():
    ...
    assert agent.accept(MemoryRequest(addr=0x10, size=4, write=True, data=0xAABBCCDD, mask=0b0101, txn_id=1)) is False
    assert agent.tick() == []
    assert agent.accept(MemoryRequest(addr=0x10, size=4, write=True, data=0xAABBCCDD, mask=0b0101, txn_id=1)) is True
