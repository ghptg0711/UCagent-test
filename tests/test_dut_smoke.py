...
async def test_write_then_read(setup_dut):
    ...
    assert resp.txn_id == read_txn.txn_id, (
        f"txn_id mismatch: expected {read_txn.txn_id}, got {resp.txn_id}"
    )
    assert resp.data == write_data, (
        f"Read data mismatch: expected 0x{write_data:x}, got 0x{resp.data:x}"
    )
...
async def test_read_miss_then_hit(setup_dut):
    ...
    assert second_resp.data == first_data, (
        "Second read should return same data as first (cache hit)"
    )
...
async def test_partial_write_with_mask(setup_dut):
    ...
    assert resp.data == expected_data, (
        f"Partial write data mismatch: expected 0x{expected_data:x}, got 0x{resp.data:x}"
    )


async def test_adapter_data_path_against_reference(setup_dut):
    """Compare adapter read data with the reference model."""
    _dut, adapter = setup_dut
    reference = ReferenceCache()

    write_txn = CacheTxn(
        op=CacheOp.WRITE, addr=0x5000, size=8, data=0xDEADBEEFCAFEBABE, mask=0xFF, txn_id=1
    )
    reference.access(write_txn)
    await adapter.drive_cpu_request(write_txn)
    write_response = await adapter.sample_cpu_response()
    assert write_response.txn_id == write_txn.txn_id
    assert write_response.error is None

    read_txn = CacheTxn(op=CacheOp.READ, addr=0x5000, size=8, txn_id=2)
    expected_resp = reference.access(read_txn)
    await adapter.drive_cpu_request(read_txn)
    actual_resp = await adapter.sample_cpu_response()

    assert actual_resp.data == expected_resp.data, (
        f"Data mismatch: expected 0x{expected_resp.data:x}, got 0x{actual_resp.data:x}"
    )
