...
class SemanticRefactoringTracker:
    ...
    def _analyze_coverage_refactoring(self) -> list[RefactoringEvent]:
        ...
        events.append(
            RefactoringEvent(
                ...
                modification_level=ModificationLevel.ARCHITECTURE,
                entropy_score=0.85,
                description="Extended coverage model with 12 cross-coverage bins (size x mask, replacement x type, access x latency, policy, address pattern)",
    ...
    def _compute_metrics(self) -> None:
        ...
        for _module, metrics in self.module_metrics.items():
    ...
    def _generate_summary(self) -> dict[str, Any]:
        ...
        arch_events = sum(
            1
            for e in self.refactoring_events
        ...
        syntax_events = sum(
            1 for e in self.refactoring_events if e.modification_level == ModificationLevel.SYNTAX
        )
